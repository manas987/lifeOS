...
export function SubscriptionsCard({ data }: { data: SubscriptionItem[] }) {
  return (
    <div className="glass-card p-3 pt-0 rounded-3xl w-full flex flex-col gap-3 ">
      {/* Header */}
...
export function RecentTransactionsCard({ data }: { data: TransactionItem[] }) {
  ...
  return (
    <div className="glass-card p-4 rounded-3xl w-full flex flex-col gap-3">
      ...
      {/* Header */}
      <div className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr] text-sm text-black/50 px-2">
        <span>Title</span>
        <span>Amount</span>
        <span>Category</span>
        <span>Date</span>
      ...
      {/* Rows */}
      <div className="flex flex-col h-[384px] overflow-y-auto pr-1">
        {rows.length === 0 ? (
          ...
          rows.map((t, i) => (
            <div
              key={i}
              className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr] items-center px-2 py-2 rounded-lg hover:bg-black/5 text-sm">
              {(
                [
                  "title",
                  "amount",
